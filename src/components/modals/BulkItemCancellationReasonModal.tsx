...
export const BulkItemCancellationReasonModal: React.FC<BulkItemCancellationReasonModalProps> = ({
  ...
  bulkItemCancellationOtherReason,
  bulkItemCancellationReason,
  currentUser,
  itemsSelectedForCancellation,
  pending,
  selectedTable,
  selectedTenant,
  setBulkItemCancellationOtherReason,
  setBulkItemCancellationReason,
        ...
        <IonToolbar style={{ "--background": "#1e293b", "--color": "white" }}>
          ...
          <IonButtons slot="end">
            <IonButton
              ...
              onClick={() => setShowBulkItemCancellationReasonModal(false)}
            >
              Cerrar
            </IonButton>
          </IonButtons>
      ...
      <IonContent className="ion-padding bg-slate-50">
        <div className="space-y-4">
          <p className="text-sm font-bold text-slate-600">
            Selecciona el motivo para cancelar los{" "}
            {itemsSelectedForCancellation.length} productos:
          </p>
          ...
          {bulkItemCancellationReason === "Otro" && (
            <textarea
              ...
              onChange={(e) =>
                setBulkItemCancellationOtherReason(e.target.value)
              }
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                const itemsToCancelDetails = itemsSelectedForCancellation.map(
                  (selected: any) => {
                    let name = "Producto desconocido";
                    let quantity = 1;
                    selectedTable?.comandas?.forEach((c: any) => {
                      if (c.folio === selected.folio) {
                        c.items?.forEach((it: any) => {
                          if (
                            it.product.id === selected.productId &&
                            it.plate === selected.plate
                          ) {
            ...
            disabled={
              ...
              !bulkItemCancellationReason ||
              (bulkItemCancellationReason === "Otro" &&
                !bulkItemCancellationOtherReason.trim())
            }
