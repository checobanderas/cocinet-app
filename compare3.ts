...
async function main() {
  const fileContent = fs.readFileSync('expected_products.txt', 'utf8');
  // Handle UTF16-LE BOM from Out-File by removing null bytes
  const cleanedContent = fileContent.replace(/\0/g, '');
  const expectedLines = cleanedContent.split('\n').map(l => l.trim()).filter(l => l.length > 0);
  ...
  for (const tenant of tenants) {
    ...
    const dbProducts = snap.docs.map(d => ({ id: d.id, ...d.data() as any }))
    ...
    expectedMap.forEach((ep, normName) => {
      if (dbMap.has(normName)) {
        ...
        let differences = [];
        if (Number(dp.price) !== ep.price) differences.push("Precio (esperado $, actual $)");
        // We will skip order comparison if it's not present in db, but let's check it just in case
        // if (dp.order !== undefined && Number(dp.order) !== ep.order) differences.push("Orden (esperado $ep.order, actual $dp.order)");
