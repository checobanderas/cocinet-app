...
<table id="example" class="display stripe row-border order-column" width="100vw" tabindex="0">
        <thead>
            <tr>
...
$campos=array("ID","NOMBRE","FOLIO","FECHA","IMPORTE","IVA","SUBTOTAL","ISR","TOTAL","UUID","ESTADO");
...
$sql='
SELECT `ID_FACTURA`, `NOMBRE`, `FOLIO`, `FECHA`,TIMPORTE, `IVA`, `SUBTOTAL`,ISR, `TOTAL`, UUID, F.estado, ID FROM facturas F INNER JOIN clientes C on F.ID_CLIENTE= C.ID  ORDER BY ID_FACTURA DESC';
...
parent.document.title='<?php  echo 'Factura MOBI Ver 4.0'; ?>' ;
...
var resultVal = 0.0; 
 ...
 var resultVal = 0.0; 
   ...
   // alert(resultVal);
$('*').find($('#pie4')).html(resultVal.toFixed(2)); 
 // $('#pie4').html(resultVal.toFixed(2));   
$('.dataTables_scrollFootInner > table:nth-child(1) > tfoot:nth-child(1) > tr:nth-child(1) > th:nth-child(5)').html(resultVal.toFixed(2)); 
...
$('.dataTables_scrollFootInner > table:nth-child(1) > tfoot:nth-child(1) > tr:nth-child(1) > th:nth-child(6)').html(resultVal.toFixed(2)); 
...
$('.dataTables_scrollFootInner > table:nth-child(1) > tfoot:nth-child(1) > tr:nth-child(1) > th:nth-child(7)').html(resultVal.toFixed(2)); 
...
$('input.form-control').attr('placeholder','DA CLICK AQUI PARA BUSCAR');
 </script>