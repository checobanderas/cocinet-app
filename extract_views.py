...
# Pre-parse all available ionic components and icons from App.tsx imports to know what to look for
...
def extract_render_function(function_name, component_name):
    ...
    match = re.search(r'const\s+' + function_name + r'\s*=\s*\((.*?)\)\s*=>\s*\{', body_lines[0])
    func_params_str = match.group(1) if match else ""
    ...
    # Find dependencies (state vars, other functions)
    state_vars = set()
    ...
    # Props are external dependencies that are not passed as arguments to this function
    ...
    # Find used components and icons
    used_components = ionic_components & body_words
    used_icons = ionic_icons & body_words
    
    # logo icons are special in ionicons (e.g. logoWhatsapp)
...
{props_pass_str}
    ...
    # Important: update 'content' and 'lines' for the next extraction
    content = new_content
    lines = content.split('\n')
...
extract_render_function('renderMenu', 'MenuView')
extract_render_function('renderCheckout', 'CheckoutView')
